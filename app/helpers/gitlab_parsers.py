...
from app.helpers.gitlab_client import get_kus, get_readme
...
def parse_ku(headers, ku):
    ...
    reactions_url = app.config['GITLAB_URL'] + "/api/v4/projects/" + str(
        projectid
    ) + "/issues/" + str(kuid) + "/award_emoji"
    reactions_response = (
        requests.request(
            request.method,
            reactions_url,
    ...
    contribution_url = app.config['GITLAB_URL'] + "/api/v4/projects/" + str(
        projectid
    ) + "/issues/" + str(kuid) + "/notes"
    ...
    if contribution_response.status_code == 200:
        contributions = [
            parse_contribution(headers, x)
            for x in contribution_response.json()
        ]
    else:
        contributions = []
...
def parse_contribution(headers, contribution):
    return {
        ...
        'metadata': {
            'author': contribution[
                'author'],  #parse_user(headers, contribution['author']['id']),
